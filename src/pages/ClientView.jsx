...
const ClientView = ({ forceModule }) => {
  ...
  const { onMouseDown, onMouseMove, selectionBox, isDragging } = useDragSelect({
    items: [], // Hook uses DOM query but items param is for reactivity if needed
    selectedIds,
    setSelectedIds,
    ...
    const fetchClientBySlug = async () => {
      try {
        ...
        if (isSuperAdmin) {
          ...
          clientData = response.data.find(c => slugify(c.company_name) === companySlug);
        } else {
          // If not superadmin, the user can only view their own content
          const userAuthData = JSON.parse(localStorage.getItem('user'));
          if (userAuthData && slugify(userAuthData.company_name) === companySlug) {
            clientData = userAuthData;
          }
        }
  ...
  const getValidImageUrl = (url) => {
    ...
    // Generic relative path – prefix with server base
  ...
  const handleModuleSelect = async (module, shouldNavigate = true) => {
    setSelectedIds([]); // Clear selection on module change
    if (shouldNavigate) {
  ...
  const handleBulkDelete = async () => {
    ...
    // Use DeleteConfirmationModal for bulk delete too
    setItemToDelete({ id: 'bulk', isFolder: activeModule === 'videos' || activeModule === 'images' });
    ...
    try {
      if (itemId === 'bulk') {
        ...
        for (const id of selectedIds) {
          let endpoint = '';
          if (activeModule === 'blogs') endpoint = `/blogs/${id}`;
          else if (activeModule === 'news') endpoint = `/news/${id}`;
          else if (activeModule === 'videos') endpoint = `/videofolders/${id}`;
          else if (activeModule === 'images') endpoint = `/imagefolders/${id}`;
  ...
  const handleDelete = (item, isFolder = false) => {
    ...
    // Always use two-step for blogs, news, and folders
    const isTwoStep = isFolder || activeModule === 'blogs' || activeModule === 'news';
    setIsTwoStepDelete(isTwoStep);
    setIsDeleteModalOpen(true);
  };
  ...
  return (
    <div className="animate-fade-in w-full px-4 md:px-8">
      <Toast
      ...
      {isSuperAdmin && (
        <div className="relative h-48 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-t-2xl overflow-hidden">
          ...
          <div className="absolute bottom-6 left-6 flex items-end gap-6 text-white z-10">
            <div className="w-24 h-24 bg-white/30 rounded-xl shadow-lg p-2 overflow-hidden flex items-center justify-center">
              <img
                src={getValidImageUrl(client.logo)}
                alt="Logo"
                className="w-full h-full object-contain"
                style={{ display: getValidImageUrl(client.logo) ? 'block' : 'none' }}
                onError={(e) => {
              ...
              <div
                ...
                style={{ display: getValidImageUrl(client.logo) ? 'none' : 'flex' }}
              >
      ...
      {/* Conditional top rounding if header is missing */}
      <div className={isSuperAdmin ? 'bg-white shadow-sm border border-slate-200 p-8 rounded-b-2xl border-t-0 mb-8' : ''}>
        ...
        {activeModule && (
          <div
            className="mt-6 animate-slide-up bg-slate-50/30 rounded-3xl p-6 border border-slate-100"
          ...
          >
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-200/50">
              ...
              >
                ...
                <span className="uppercase tracking-widest">
                  {activeModule === 'blogs' || activeModule === 'news' ? `Add ${activeModule.slice(0, -1)}` : 'Add Folder'}
                </span>
              </button>
            </div>
            ...
            {selectedIds.length > 0 && (
              <div className="mb-6 flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-xl shadow-slate-100 animate-slide-up">
                <div className="flex items-center gap-4">
                  <div className="bg-blue-600 text-white px-3 py-1 rounded-lg text-xs font-black tracking-widest">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-red-50 text-red-600 border border-red-100 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all flex items-center gap-2"
                >
            ...
            {moduleLoading ? (
              ...
              <div className={(activeModule === 'blogs' || activeModule === 'news') ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full" : "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-12 gap-6 xl:auto-rows-[220px] w-full"}>
                ...
                {activeModule === 'videos' &&
                  moduleData.map((folder, index) => {
                    let colSpan = "xl:col-span-4";
                    let rowSpan = "xl:row-span-1";

                    if (index === 0) {
                      colSpan = "xl:col-span-8";
                    ...
                    return (
                      <div
                        ...
                        className={`selectable-item relative group rounded-[2.5rem] overflow-hidden border ${selectedIds.includes(folder.id) ? 'border-blue-500 ring-4 ring-blue-100 shadow-xl' : 'border-slate-100'} shadow-sm hover:shadow-2xl transition-all duration-700 bg-white ${colSpan} ${rowSpan} min-h-[220px] flex flex-col`}
                      >
                        {/* Selection Checkbox */}
                        <div className="absolute top-4 left-4 z-20">
                          <button
                            ...
                            className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all ${selectedIds.includes(folder.id)
                              ? 'bg-purple-600 border-purple-600 text-white shadow-lg shadow-purple-200'
                              : 'bg-black/40 backdrop-blur-md border-white/20 text-transparent hover:border-purple-400'
                              }`}
                          >
                        ...
                        <div className="relative z-10 flex-1 flex flex-col justify-end p-6 pointer-events-none">
                          <div className="flex items-end justify-between">
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 pointer-events-auto">
                              <button
                  ...
                  moduleData.map((folder, index) => {
                    ...
                    return (
                      <div
                        ...
                        data-id={folder.id}
                        style={{ userSelect: 'none' }}
                        className={`selectable-item relative group rounded-[2.5rem] overflow-hidden border ${selectedIds.includes(folder.id) ? 'border-teal-500 ring-4 ring-teal-100 shadow-xl' : 'border-slate-100'} shadow-sm hover:shadow-2xl transition-all duration-700 bg-white ${colSpan} ${rowSpan} min-h-[220px] flex flex-col`}
                      >
                        {/* Selection Checkbox */}
                        <div className="absolute top-4 left-4 z-20">
                          <button
                            ...
                            className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all ${selectedIds.includes(folder.id)
                              ...
                              : 'bg-white/40 backdrop-blur-md border-white/20 text-transparent hover:border-teal-400'
                              }`}
                          >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                          ...
                          <div className="flex items-end justify-between">
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] text-white/70 font-bold uppercase tracking-[0.3em] drop-shadow-md">
                                {folder.images?.length || 0} Images <span className="mx-1">•</span> View Folder
                              </p>
                            </div>
            ...
            {isDragging && selectionBox && (
              <div
                className="fixed border-2 border-blue-500/50 bg-blue-500/10 pointer-events-none z-[1000] rounded-lg"
                style={{
        ...
        <CreateBlogModal
          ...
          clientId={isSuperAdmin ? client?.id : undefined}
        ...
        />
        <CreateFolderModal
          isOpen={isCreateOpen && (activeModule === 'videos' || activeModule === 'images')}
        ...
        <DeleteConfirmationModal
          ...
          title={itemToDelete?.id === 'bulk' ? `Bulk Delete ${(activeModule || '').charAt(0).toUpperCase() + (activeModule || '').slice(1, -1)}?` : (itemToDelete?.isFolder ? 'Delete Folder?' : `Delete ${(activeModule || ' ').slice(0, -1).charAt(0).toUpperCase() + (activeModule || '').slice(1, -1)}?`)}
          message={itemToDelete?.id === 'bulk'
            ? `Are you sure you want to delete ${selectedIds.length} items?`
            : (itemToDelete?.isFolder
              ? `Are you sure you want to delete "${itemToDelete?.title}"?`
              : `Are you sure you want to delete this ${(activeModule || '').slice(0, -1)}?`)}
          twoStep={isTwoStepDelete}
          step1Message={{
            title: itemToDelete?.id === 'bulk' ? 'Bulk Delete Confirmation' : 'Delete Confirmation',
